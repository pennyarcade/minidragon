...
class CPUCore:
    ...
    def tick(self) -> None:
        ...
        while len(instructions) > 0:
            ...
            self.alu.b = signextend(self.b, 7)
            ...
            if instruction.imm_4_output:
                self.data = (
                    (self.data & 0xFF00) +
                    (signextend(self.ir, 3) & 0xFF)
                )
            if instruction.imm_6_output:
                self.data = (
                    (self.data & 0xFF00) +
                    (signextend(self.ir, 5) & 0xFF)
                )
...
class JRI(BaseInstruction):
    ...
    def mnemonic(self, instruction: int) -> str:
        jumppoint = bintoint(signextend(instruction & 0x3F, 5) & 0xFF)
        if jumppoint == 0:
            return "NOP"
        elif jumppoint == -1:
            return "HALT"
...
class ADDI(BaseInstruction):
    ...
    def mnemonic(self, instruction: int) -> str:
        integer = bintoint(signextend(instruction & 0x3F, 5) & 0xFF)
        if integer == 1:
            return "INC"
        elif integer == -1:
            return "DEC"
...
class PUSHIP(BaseInstruction):
    ...
    def mnemonic(self, instruction: int) -> str:
        ...
        integer = bintoint(signextend(instruction & 0x0F, 3) & 0xFF)
...
class ADDPCI(BaseInstruction):
    ...
    def mnemonic(self, instruction: int) -> str:
        ...
        integer = bintoint(signextend(instruction & 0x3F, 5) & 0xFF) + 1
        if integer == 1:
            return "INCPC"
        else:
